	...
	while ((loopcounter < times_to_ping || times_to_ping == -1) && CTRL_C_ABORT == 0 ) {
		...
		if (have_valid_target == 1) {
			...
			if (sd == INVALID_SOCKET) {
				...
				std::string abuffer;
				formatIP(abuffer, AI);				
				out.pf("探测%s:%d/tcp ", abuffer.c_str(), nPort);
			...
			out.pf( " 时间=%0.3fms毫秒 ", response_time);

			if (use_http == 1) {
				if (include_url == 1) {
					if (docptr != NULL) {
						out.pf( "页面:http://%s/%s ",pcHost,docptr);
					} else {
						out.pf( "页面:http://%s ", pcHost);
					}
				}

				out.pf("接收时间=%0.3f毫秒 状态=%d 接收字节数=%d ", http_response_time, http_status, bytes_received);

				bps = bytes_received * 1000 / http_response_time;
				bps = bps * 8 / 1000;

				out.pf("速度=%0.3fkbit/s ",bps);
			}
			...
			if (include_jitter == 1 && success_counter > 1) {
				if (jitter_sample_size == 0) {
					...
					out.pf( "抖动=%0.3f ", response_time - ( (running_total_ms - response_time) / (success_counter - 1)));
					if (use_http == 1) {
						out.pf( "接收抖动=%0.3f ", http_response_time - ( (running_total_ms_http - http_response_time) / (success_counter - 1) ));
					}
					...
					out.pf( "抖动=%0.3f ", response_time - (j / min(success_counter -1, jitter_sample_size) ));
					...
					if (use_http == 1) {
						...
						out.pf( "接收抖动=%0.3f ", http_response_time - (j / min(success_counter -1, jitter_sample_size) ));
					}
				}
			}
